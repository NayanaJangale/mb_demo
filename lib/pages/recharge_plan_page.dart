...
class RechargePlanPage extends StatefulWidget {
  ...
  @override
  _RechargePlanPageState createState() => _RechargePlanPageState();
}

class _RechargePlanPageState extends State<RechargePlanPage> with TickerProviderStateMixin{
  final GlobalKey<ScaffoldState> _rechargePlanPageGlobalKey =
  ...
  void initState() {
    ...
    Pay2NewAPI(context: this.context)
        .getROffer(
        Number: widget.subscriberNo,
        OperatorCode: widget.OpCode )
        .then((res) {
      setState(() {
        _isLoading = false;
      });
      if (res != null && HttpStatusCodes.OK == res['Status']) {
        ...
        setState(() {
          rechargePlanList =
          List<RechargePlan>.from(offerdata.map((i) => RechargePlan.fromMap(i)));
        });
      } else {
      ...
      child: Container(
        ...
        child: SafeArea(
          child: Scaffold(
              ...
              key: _rechargePlanPageGlobalKey,
              body: Column(
                                                ...
                                                children: <Widget>[
                                                  SizedBox(
                                                    ...
                                                    child: Padding(
                                                      ...
                                                      child: Text( '\u{20B9}' + rechargePlanList[index].rs,style: Theme.of(context)
                                                          .textTheme
                                                          .bodyText1
                                                          .copyWith(
                                        ...
                                        switch (widget.menuFor){
                                          ...
                                          case MenuName.DTHRecharge:
                                            Navigator.pushReplacement(
                                              context,
                                              MaterialPageRoute(
                                                  builder: (_) => MobileRechargePage(subscriberNo: widget.subscriberNo,menuFor: widget.menuFor,circleCode: widget.circleCode,paymentStatus: widget.paymentStatus,operatorCode: widget.operatorCode,amount: rechargePlanList[index].rs,serviceType: widget.serviceType,serviceName:widget.serviceName,menuType: widget.menuType,)
                                              ),
                                            );
                                            break;
                                        }
